...
from Misc.tracking import get_tracker, WandBTrackerNonINIT
...
def train_model(wandb_config = None, mark_preempting = True, tag = None):
    ...
    with wandb.init(config=wandb_config, project=config.project) as run:
        ...
        config_cmd_line_dict = {}
        for key in wandb_config.keys():
            config_cmd_line_dict [f"--{key}"] = str(wandb_config[key])

        args = parse_args(config_cmd_line_dict)
        main(args, WandBTrackerNonINIT(None, None))
