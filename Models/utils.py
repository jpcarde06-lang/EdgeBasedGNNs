...
def edge_level_pool(x, edge_batch, node_batch):
    # Mean pool edges per graph
    pooled = global_mean_pool(x, edge_batch)
    
    # Count nodes per graph
    num_nodes = torch.bincount(node_batch)
    
    # Scale pooled edge embedding by number of nodes
    scaled = pooled * num_nodes.unsqueeze(1)
...
def get_pooling_fct(readout):    
    ...
    elif readout == "nodesum":
        return edge_level_pool
    else:
