...
class WandBTrackerNonINIT(Tracker):
    """
    WandB Tracker that does not init WandB tracking so Umibozu can do it for us
    """
    def __init__(self, exp_config, project_name):
...
class WandBTracker(WandBTrackerNonINIT):
    def __init__(self, exp_config, project_name):
        super(WandBTracker, self).__init__(exp_config, project_name)
        import wandb
        wandb.init(
            config = exp_config,
